	...
	public class PackageListView : MonoBehaviour {
		...
		IEnumerator LoadPackageList() {
			...
			if (GameState.isShowBuiltins) {
				...
				Package[] packages = Repository.shared.GetBuiltinPackages(Settings.GetString(Settings.PACKAGE_LANGUAGE, Settings.SUPPORT_PACKAGE_LANGUAGES));
				...
				Package[] packages = Repository.shared.GetUGCPackages(Settings.GetString(Settings.PACKAGE_LANGUAGE, Settings.SUPPORT_PACKAGE_LANGUAGES));
				InitPackageList(packages);
			}
		}
