...
namespace PuzzleSister.UGCEditor
{
    ...
    public class UGCQuestionService
    {
        ...
        private void Save()
        {
            ...
            var packageProgressItem = PackageProgressService.shared.GetProgress(package.id);
            if (packageProgressItem == null) {
                PackageProgressService.shared.SetProgress(package.id, 0, questionList.Count);
            } else {
                PackageProgressService.shared.SetProgress(package.id, packageProgressItem.progress, questionList.Count);
            }
            string questionPath = Utils.Path(contentDir, Const.QUESTION_FILENAME);
